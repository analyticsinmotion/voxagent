...
// The addon resolves every call with a result, whether or not the model loaded
// and inference ran, so the lines written during a call are the only signal that
// it failed. These patterns match the lines written on a failure and no line
// written in ordinary use.
const FAILURE_LINES = [
  // The addon's own lines. It writes "error: failed to initialize whisper context"
  // for every call made without a loaded model, and "failed to process audio"
  // when inference fails.
  /^error: /i,
  /^failed to process audio$/,
  // whisper's lines while it loads a model. A device backend that fails to start
  // is reported as "failed to initialize <name> backend" and whisper continues on
  // the CPU, so that line is not a failure.
  /^whisper_\w+: ERROR /,
  /^whisper_\w+: failed to (?!initialize \S+ backend$)/,
  /^whisper_\w+: \w+\(\) failed/,
  /^whisper_\w+: invalid model/,
  /^whisper_\w+: (unknown tensor '|tensor '.*' has wrong |shape: \[)/,
];
...
function failureLines(text) {
  return text.split(/\r?\n/).filter((line) => FAILURE_LINES.some((pattern) => pattern.test(line)));
}

// The addon's binaries

// The addon's dist directory. require.resolve locates the package without running it.
function whisperDist() {
  return path.join(path.dirname(require.resolve(WHISPER_PACKAGE)), '..');
}
...
// The addon's Linux and macOS binaries need libraries shipped beside them, such as
// libwhisper.so.1 and @rpath/libwhisper.1.dylib, but the only library search path they
// carry is the directory they were built in, so the dynamic loader cannot find those
// libraries. The loader reads LD_LIBRARY_PATH on Linux and DYLD_LIBRARY_PATH on macOS as
// a process starts, and searches it before that build directory, so the child is started
// with the binary's own directory first in that variable. An existing value is kept
// after it. Nothing is appended when there is no existing value, because an empty entry
// in LD_LIBRARY_PATH names the current directory. On Windows a library is found in the
// directory of the module that loads it, so the environment is returned unchanged.
const LIBRARY_PATH_VARIABLES = { linux: 'LD_LIBRARY_PATH', darwin: 'DYLD_LIBRARY_PATH' };

function childEnvironment(env, platform, arch) {
  const variable = LIBRARY_PATH_VARIABLES[platform];
  const binary = variable ? whisperBinary(whisperDist(), platform, arch) : null;
  ...
  return { ...env, [variable]: existing ? `${dir}:${existing}` : dir };
}
...
function start(options) {
  ...
  const child = fork(__filename, [], { silent: true, env: childEnvironment(process.env, process.platform, process.arch) });
...
module.exports = { start, failureLines, whisperDist, whisperBinary, childEnvironment };
