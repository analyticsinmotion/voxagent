...
// Destroys the write stream, waits for it to close its file, and only then removes the
// temporary file. A response that ends early never ends the stream it is piped into, so
// the file would otherwise stay open for as long as the process runs, and on Windows a
// file removed while it is open can stay in its directory until it is closed. A file
// that is already gone is not an error. Any other failure to remove it is added to the
// download error. Resolves with the error to report.
function discard(file, tmpDest, err) {
  return new Promise((resolve) => {
    const remove = () => {
      fs.unlink(tmpDest, (unlinkErr) => {
        if (!unlinkErr || unlinkErr.code === 'ENOENT') {
          resolve(err);
          return;
        }

        resolve(new Error(`${err.message} The partial download ${tmpDest} could not be removed: ${unlinkErr.message}`, { cause: err }));
      });
    };
  ...
  return new Promise((resolve, reject) => {
    ...
    proto.get(url, (res) => {
      if (REDIRECT_STATUSES.includes(res.statusCode) && res.headers.location) {
        ...
        // HTTP allows a relative Location, so it is resolved against the URL of the
        // request that returned it. A Location that is not a valid URL rejects here,
        // and a redirect with no Location fails below on its status.
        let next;
        try {
          next = new URL(res.headers.location, url).href;
        } catch (err) {
          return reject(new Error(`Download failed: HTTP ${res.statusCode} redirected to an invalid location`));
        }

        return download(next, dest, expected, maxRedirects - 1).then(resolve, reject);
      }
      ...
      const fail = (err) => {
        ...
        failed = true;
        discard(file, tmpDest, err).then(reject);
      };
      ...
      file.on('finish', () => {
        file.close(() => {
          ...
          if (problem) {
            return fail(new Error(`${problem}. Run voxagent again to download it again.`));
          }
      ...
      res.on('error', (err) => {
        fail(new Error(`download did not complete, ${err.message}. Run voxagent again to download it again.`));
      });
    }).on('error', reject);
  });
}
